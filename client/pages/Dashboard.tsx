  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      ...
      <section className="mb-8 bg-white rounded shadow p-6">
        <h2 className="text-xl font-bold mb-4">Registry Summary</h2>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="text-center"><div className="text-2xl font-bold">{summary.totalItems}</div><div className="text-gray-600 text-sm">Total Items</div></div>
          <div className="text-center"><div className="text-2xl font-bold">{summary.fullyContributed}</div><div className="text-gray-600 text-sm">Fully Contributed</div></div>
          <div className="text-center"><div className="text-2xl font-bold">{summary.partiallyContributed}</div><div className="text-gray-600 text-sm">Partially Contributed</div></div>
          <div className="text-center"><div className="text-2xl font-bold">{summary.delivered}</div><div className="text-gray-600 text-sm">Delivered</div></div>
          <div className="text-center"><div className="text-2xl font-bold">₦{summary.totalContributed.toLocaleString()}</div><div className="text-gray-600 text-sm">Total Contributed</div></div>
        </div>
      </section>

      {/* Section 2: Quick Links */}
      <section className="bg-white rounded shadow p-6 flex flex-col gap-4 items-center justify-between mb-8">
      ...
      <section className="mb-8 bg-white rounded shadow p-6">
        <h2 className="text-xl font-bold mb-4">Registry Details</h2>
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <div className="flex gap-2 mt-1">{details.photos.length === 0 ? <span className="text-gray-400">No photos</span> : details.photos.map((url, i) => <img key={i} src={url} alt="Wedding" className="w-16 h-16 object-cover rounded" />)}</div>
          </div>
          <button className="text-[#B8860B] underline">Edit</button>
        </div>
        <div className="mb-4 flex items-center justify-between">
      ...
      <section className="mb-8 bg-white rounded shadow p-6">
        ...
        <h2 className="text-xl font-bold mb-4">Gift Redemption Options</h2>
        <select
        ...
        {(redemption === 'immediate' || redemption === 'after-wedding') && (
          <div className="mb-4">
            ...
            {redemption === 'after-wedding' && provideLater && (
              <div className="mt-2 text-green-700 font-semibold">We will reach out to you 2 weeks before your wedding date to collect your account information.</div>
            )}
          </div>
        )}
        {redemption === 'physical' && (
          <div className="mb-4 text-green-700 font-semibold">We will reach out to you to arrange delivery of your gifts.</div>
        )}
        <div className="mt-2 text-gray-700 text-sm">You can update this setting anytime. If you need additional support, please <a href="/contact" className="underline text-[#B8860B]">contact us</a>.</div>
        <button className="px-4 py-2 bg-[#B8860B] text-white rounded hover:bg-[#8B6508]">Save Redemption Option</button>
      </section>
    </div>
  );
};
